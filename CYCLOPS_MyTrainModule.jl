...
macro myepochs(n, ex)
  ...
  @progress for i = 1:$(esc(n))
      @info "Epoch $i"
      avgloss = $(esc(ex))
      if length(lossrecord) > 5 && avgloss > avgloss[length(lossrecord)] && avgloss[length(lossrecord)] > avgloss[length(lossrecord) - 1] && avgloss[length(lossrecord) - 1] > avgloss[length(lossrecord) - 2] && avgloss[length(lossrecord) - 2] > avgloss[length(lossrecord) - 3]
        break
      else
